...
export const zarinPalWebhookHandler: RouteHandler<typeof zarinPalWebhookRoute, ApiEnv> = async (c) => {
  // Custom security validations for webhook
        ...
        try {
          ...
          if (verification.data?.code === 100 || verification.data?.code === 101) {
            ...
            if (paymentRecord.subscriptionId) {
              const subscriptionData = await billingRepositories.subscriptions.findById(paymentRecord.subscriptionId);

              if (subscriptionData && subscriptionData.status === 'pending') {
                const startDate = new Date();
                const nextBillingDate = subscriptionData.billingPeriod === 'monthly'
                  ? new Date(startDate.getTime() + 30 * 24 * 60 * 60 * 1000)
                  : null;

                await billingRepositories.subscriptions.update(subscriptionData.id, {
                  status: 'active',
                  startDate,
                  nextBillingDate,
  ...
  if (c.env?.EXTERNAL_WEBHOOK_URL) {
    try {
      ...
      // Production-ready fetch with timeout for webhook forwarding
      const fetchConfig: FetchConfig = {
        timeoutMs: 15000, // 15 second timeout for webhook forwarding
        maxRetries: 2, // Retry for transient failures
      ...
      await db
        ...
        .set({
          ...
          forwardingError: forwarded ? null : (fetchResult.success ? 'No error' : fetchResult.error) || 'Unknown error',
        })
        .where(eq(webhookEvent.id, eventId));
    } catch (forwardError) {
...
export const testWebhookHandler: RouteHandler<typeof testWebhookRoute, ApiEnv> = async (c) => {
  ...
  // Production-ready fetch with timeout for webhook testing
  const fetchConfig: FetchConfig = {
    timeoutMs: 10000, // 10 second timeout for test webhooks
    maxRetries: 1, // Single retry for test
    correlationId: crypto.randomUUID(),
  };
